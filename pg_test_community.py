...
from dataset import generate, convert_to_s2vgraph, generate_community
...
np.savetxt("graph_edges.txt", edges.numpy(), fmt='%i')
avg_mask, coms = find_communities("graph_edges.txt")

com_idx = np.random.randint(0, len(coms))
print(coms)
print(coms[com_idx])
...
dataset, path = generate_community(500, nodes_per_graph_nr, sigma, graph, coms[com_idx], no_of_features)
#path = 'graphs_3_11'
    ...
    for data in train_dataloader:
        ...
        for i in range(data.y.size(0)):
            for j in range(nodes_per_graph_nr):
                batch.append(i)
        logits = mutag_model(data, torch.tensor(batch))
